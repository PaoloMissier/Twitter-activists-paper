...
\anote{the definition of activists is based on a few established metrics. we do not invent new metrics but we combine a few that exist in a way that is justified intuitively, by drawing from literature on online activism.

the metrics include (1) non-topological metrics based on a user's post history

(2) topological, that is based on static  as well as dynamic relationships amongst users.

many metrics have been proposed in the past.
...
``Activists'' have not been proposed -- but they are useful to find. 

so we begin by proposing an operational definition of activists.
}

Our research hypotheses is that we can efficiently harvest activists' profiles from Twitter, at the granularity of single individuals, and associate a \textit{strength} to their profiles based on their recurring online presence over time.
...
\anote{these are roughly identified with events, campaigns, or other online context}
