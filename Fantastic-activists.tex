...
\anote{FLAVIO:
	\begin{itemize}
		...
		\item brief description of the twitter api
		\item how query for each event is composed
		\item how edges are created (direction and weigths) and resulting network type
	\end{itemize}
}

\subsection{Community detection}  \label{sec:communities}

\anote{describe Demon in summary}

\anote{FLAVIO:
	\begin{itemize}
	    \item briefly describe this step purpose
		\item brief description of community detection concept
		\item how demon works
		\item why choose demon: ego-network, multiple communities
		\item indegree centrality wrt communities
		\item mention filtering out of users with no community
	\end{itemize}
}
