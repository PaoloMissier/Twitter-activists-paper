...
Tweets are used to create a network graph which describes the relation among the users in the given context.
Tweets about a context are obtained using the Twitter API 1.1 \anote{citation to twitter api 1.1 doc needed?}.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item each n gets gets the possibility to change its label with probability $\alpha$ based on the label majority of its neighbours.
...
In-degree centrality is then calculated for each node locally inside each found community.
...
\anote{FLAVIO:
	\begin{itemize}
	    \item briefly describe this step purpose
		\item collection of profile data from each user on twitter
		\item metrics on collected profile data: follower rank (popularity)
		
		\item collection of timeline data during the event from each user on twitter
		\item metrics on collected timeline data: topical attachment, event focus, topical strength
		
		\item apply ranking functions on multiple features
		\item list possible ranking functions
	\end{itemize}
}
...
\anote{FLAVIO:
	\begin{itemize}
		\item briefly describe this step purpose
		\item how context discovery in manually done
	\end{itemize}
}
