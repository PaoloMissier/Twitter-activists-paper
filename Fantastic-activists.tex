...
As described In Sec. ~\ref{sec:reference}, contexts are meant to identify events or campaigns around social issues, which are characterised by spatio- and/or temporal boundaries \anote{FLAVIO: conflicts with previous definition which says that are temporally defined and can be optionally also defined spatially} and by hashtags and/or keyword terms.
...
Each tweet $t \in T$ is an instance of one of these possible activities: an \textit{original tweet}, a \textit{retweet}, and a \textit{mention}. \anote{FLAVIO: original tweet or retweet (need to check this one also) only}
...
\anote{FLAVIO: using t\_1 and t\_2 for time intervals and t for tweets may be misleading?}
...
\end{enumerate}
\anote{FLAVIO: need to better clarify examples also?}
...
\anote{process structure (use one of Flavio's figures: \\
...
\noindent combine all metrics using a ranking function (which is a parameter in our framework)\\
rank \\
remove white list users \\
threshold (parameter) \\
update user profiles in DB\\

\noindent also use the profiles to discover new contexts (automated hashtag extraction followed by manual selection step)
