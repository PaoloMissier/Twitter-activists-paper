...
We have clarified in the introduction that our approach to discovering interesting users is \textit{unsupervised}, as the usual approach of having experts manually generate ground truth is vulnerable to the subjectivity of the experts. Thus, the evaluation would simply be measuring the fit of the model to the specific experts' own opinion. 
Instead, our pipeline creates a feature-rich database of online profiles that are pre-selected according to specific topological properties in order to filter out background noise, but are otherwise objectively represented and ready to be mined using any user-defined function of the available features.

Thus, our evaluation shows that the process is effective in building up such a database, by (i) showing the pipeline in action on a significant set of contexts, and (ii) demonstrating user-defined ranking functions that operate on the resulting database.
Specifically, we  start by manually selecting meaningful contexts from the same social domain, namely public healthcare campaigns, to demonstrate network construction, breakdown of each  context network into communities, and harvesting of users from each community, along with their metrics.
We then provide examples of ranking functions that we believe to capture the notion of  \textit{online activists}, and we empirically validate them by inspecting the profiles of representative top-k users.

 \subsection{Contexts, networks, and communities}  \label{sec:contexts}
 

 We have manually selected 25 contexts within the scope of healthcare campaigns in the UK, all occurring in 2018 (Table~\ref{tab:contexts}). 
 Table~\ref{tab:networks-summary} provides summary statistics for  the corresponding user-user networks constructed by harvesting posts for each context.
...
\begin{table}
\begin{center}
	\includegraphics[width=0.7\linewidth]{figures/summary-events}
	\end{center}

\anote{omit location as it's all UK}		
	\caption{List of contexts used in the experiments}
	\label{tab:contexts}
\end{table}	 
...
\begin{table}
	\begin{center}
		\includegraphics[width=0.7\linewidth]{figures/summary-events-graphs.png}
	\end{center}
	
		Summary of networks, retain these metrics   \\
	(min, max, avg, std dev) for no\_edges, 	no\_nodes, density, avg\_degree, assortativity\\
	
	use a different way to report SSC: SSC as counted here have no meaning, what matters is the ratio of SSC to no_nodes	
	
	\caption{Summary statistics for the user-user networks for all the contexts in Table~\ref{tab:contexts}}
	\label{tab:networks}
\end{table}	 
...
\begin{center}
	...
	\includegraphics[width=0.7\linewidth]{figures/summary-networks-stats}
\end{center}
	...
	\caption{Summary statistics of communities extracted from the context networks}
	\label{tab:commiunities}
\end{table}	 
