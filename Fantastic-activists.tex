...
A broader motivation for the approach we present here is that relatively low-profile users are hard to identify by using traditional approaches based on global content harvesting and large Twitter networks.
...
Relevance features are then computed for each user within the scope of each community. \anote{FLAVIO: also need to consider user inherent features such as popularity}
