...
%
\begin{abstract}
On social media platforms and Twitter in particular, some classes of users, such as the \textit{influencers}, have been given satisfactory operational definitions in terms of Twitter content metrics.
Other kinds of users, for instance \textit{online activists}, are less precisely defined.
We find that supervised machine learning approaches are unsatisfactory, as they produce models that reflect the judgment of a few experts, often on a limited training set.
%
To overcome this problem, we suggest a semi-supervised approach whereby a growing dataset of candidate users, selected from the general population using network analysis techniques, is made available to experts for mining, i.e., by experimenting with multiple user ranking functions.
In this paper we describe the design and implementation of a process that populates such dataset, by repeatedly harvesting \textit{contexts}, i.e., small but topical fragments of the  network from social events or campaigns with a significant footprint on Twitter.
We empirically demonstrate the process in action on a case study consisting of healthcare-related campaigns in the UK, showing how it supports the experimental definition of online activism.
