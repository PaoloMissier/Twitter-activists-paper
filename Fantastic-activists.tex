...
%		\begin{tabularx}{|X|X|X|X|X|X|X|X|X|X|X|X|}
			\hline 
			\rule[-1ex]{0pt}{2.5ex} \textbf{Context name} &  \textbf{start date} & \textbf{end date}  & \textbf{hashtags} & \textbf{assortativity} & \textbf{avg degree} & \textbf{density} & \textbf{edges} & \textbf{nodes} & \textbf{scc / nodes} & \textbf{\demon~ communities / nodes} &  \textbf{\infomap~ communities / nodes} \\ 
			\hline 
...
%	\end{tabularx}}
	\end{tabular}}
	\anote{update last column with Flavio's latest figures}
