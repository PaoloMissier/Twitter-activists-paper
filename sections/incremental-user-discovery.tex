...
The content processing pipeline operates iteratively on a set of contexts for a given afr, for instance . This set is initialised at the start of the process with contexts that are reupdated \anote{FLAVIO: not clear to me who is dinamically updated} at the end of each iteration, starting from an initial set of seed contexts.
...
Given a set $C$ of contexts as in (2), all Twitter posts $P(C)$ that satisfy the criteria set in $C$ are retrieved \anote{FLAVIO: not all tweets because of Twitter API limitations, maybe move here the Twitter API limitation problem mentioned below at the beginning of section 3.3?}, using either the Search or the Streaming \anote{FLAVIO: in our case only the search API} Twitter APIs.\footnote{Repeat queries and multiple accounts are used to get around the well-known Twitter API limitations for retrieving tweets.}
%
...
The goal of this partitioning is to further narrow the scope for the topology-sensitive metrics, namely FollowerRank (\ref{eq:FR}) \anote{FLAVIO: FR is not dependent on the network graph, it is a Context independent metric. Maybe move it to section 3.3?} and in-degree centrality (\ref{eq:IDC}), 
...
Note that users who do not belong to any community are discarded from the whole process. \anote{FLAVIO: insert the fact we decided to remove users in Infomap that belongs to communities with 3 or less users}
...
Next, user metrics as defined in Sec. ~\ref{sec:metrics} are computed (5) \anote{FLAVIO: also the Follower Rank} from the network and the user features.
%
...
To get around this problem, we have implemented a dedicated component to \textit{scrape} user information directly from their profile Web pages \anote{FLAVIO: remove scraping part because goes against Twitter TOS}, namely:
\begin{itemize}
...
All of these features are persisted to a database which is made available for ranking purposes. \anote{FLAVIO: only Context-independent data (user profile information and Follower Rank) and the Rank are affected}
