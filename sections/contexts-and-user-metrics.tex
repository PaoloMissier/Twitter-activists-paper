...
\hl{As described In} Sec.~\ref{sec:reference}, contexts are meant to identify events or campaigns around social issues, which are characterised by temporal boundaries and by hashtags and/or keyword terms, and optionally also by spatial constraints, i.e., specified using a geographical bounding box.
These are \textit{weak} contexts, because Twitter does not natively support the notion of event or campaign (unlike, for example, Facebook, Instagram \anote{FLAVIO: from what I've seen, it is not possible to define explicitly either a campaign or an event on Instagram}, or Meetup).
We denote a generic context as \anote{FLAVIO: as per Carlo corrections substitute either with \{\} or ()}
\begin{equation}
...
We also define the complement $\Tilde{P}(C)$ of $P(C)$ as the set of posts found using the same spatio-temporal constraints, but which do not contain any of the terms in $K$. More precisely, given a context $C'= \langle s, [t_1, t_2], \emptyset \rangle$ \anote{FLAVIO: as per Carlo corrections substitute either with \{\} or ()} with no terms constraints, we define $\Tilde{P}(C) = P(C') \setminus P(C)$. 
...
$V = \{ u(p) | p \in P(C) \}$, and a weighted \anote{FLAVIO: as per Carlo corrections: directed} edge $e = \langle u_1, u_2, w \rangle$ is added to $E$ for each pair of posts $p_1, p_2$ such that $u(p_1) = u_1, u(p_2) = u_2$ and 
...
Note that, given $C$, we can evaluate each of the features above with respect to either $P(C)$ or  $\Tilde{P}(C)$ independently from each other, that is, we can consider an ``on-context'' and an ``off-context'' version of each feature \anote{FLAVIO: with the exclusion of F1 and F2 which are Context-independent features}.
%
